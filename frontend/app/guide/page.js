...
function Step({ number, title, description, icon: Icon, children }) {
  return (
    <div style={{ display: 'flex', gap: 16, marginBottom: 32 }}>
      <div style={{ width: 36, height: 36, borderRadius: 8, background: 'var(--accent-glow)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: 14, fontWeight: 700, color: 'var(--accent)', fontFamily: 'var(--font-mono)' }}>
        {number}
      </div>
      <div style={{ flex: 1 }}>
        <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 6 }}>{title}</h3>
        <p style={{ fontSize: 13, color: 'var(--text-2)', lineHeight: 1.6, marginBottom: children ? 12 : 0 }}>{description}</p>
        {children}
      </div>
    </div>
  );
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0', borderBottom: '1px solid var(--border)' }}>
      ...
      <div style={{ display: 'flex', gap: 4 }}>
        {keys.map((k) => (
          <kbd key={k} style={{ padding: '2px 6px', borderRadius: 4, background: 'var(--bg-hover)', border: '1px solid var(--border)', fontSize: 11, fontFamily: 'var(--font-mono)', color: 'var(--text-3)' }}>{k}</kbd>
        ))}
      </div>
    </div>
  );
...
export default function GuidePage() {
  return (
    <div style={{ maxWidth: 700, margin: '0 auto', paddingTop: 32 }}>
      ...
      <p style={{ fontSize: 14, color: 'var(--text-2)', lineHeight: 1.7, marginBottom: 40 }}>
      ...
      {/* Quick Start */}
      <h2 style={{ fontSize: 18, fontWeight: 700, marginBottom: 20, paddingBottom: 8, borderBottom: '1px solid var(--border)' }}>Quick Start</h2>
      ...
      <Step number="1" title="Search for an entity" description="Enter a company name, UEI, CAGE code, or federal award ID in the search bar on the home page. Results pull from 6 federal data sources.">
        <Link href="/" style={{ fontSize: 13, color: 'var(--accent)', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 4 }}>
      ...
      <Step number="2" title="Launch an investigation" description="Click 'New Research' on the Investigate page. Provide a title and seed identifier. The system will run 6 pipeline steps automatically:">
        <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
          {[
            ...
            <div key={s.step} style={{ display: 'flex', gap: 10, padding: '8px 14px', borderBottom: i < 5 ? '1px solid var(--border)' : 'none', fontSize: 13 }}>
      ...
      {/* Navigation */}
      <h2 style={{ fontSize: 18, fontWeight: 700, marginBottom: 16, marginTop: 40, paddingBottom: 8, borderBottom: '1px solid var(--border)' }}>Navigation</h2>
      ...
      {/* Data Sources */}
      <h2 style={{ fontSize: 18, fontWeight: 700, marginBottom: 16, paddingBottom: 8, borderBottom: '1px solid var(--border)' }}>Data Sources</h2>
      ...
      <div className="card" style={{ padding: 0, overflow: 'hidden', marginBottom: 32 }}>
        {[
          ...
          <div key={s.name} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 14px', borderBottom: i < 5 ? '1px solid var(--border)' : 'none', fontSize: 13 }}>
            <div>
              <div style={{ fontWeight: 600, color: 'var(--text)' }}>{s.name}</div>
              <div style={{ color: 'var(--text-3)', marginTop: 2 }}>{s.desc}</div>
            </div>
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--text-3)', whiteSpace: 'nowrap', paddingLeft: 16 }}>{s.rate}</span>
          </div>
        ))}
      </div>

      {/* Trust & Transparency */}
      <h2 style={{ fontSize: 18, fontWeight: 700, marginBottom: 16, paddingBottom: 8, borderBottom: '1px solid var(--border)' }}>Trust & Transparency</h2>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 32 }}>
        {[
          ...
          <div key={item.title} style={{ display: 'flex', gap: 12, fontSize: 13 }}>
            <item.icon size={16} style={{ color: 'var(--accent)', flexShrink: 0, marginTop: 2 }} />
            <div>
              <span style={{ fontWeight: 600 }}>{item.title}:</span>
              <span style={{ color: 'var(--text-2)', marginLeft: 4 }}>{item.desc}</span>
            </div>
          </div>
        ))}
      </div>

      <div style={{ padding: '14px 16px', borderRadius: 8, background: 'var(--amber-glow)', border: '1px solid rgba(245,158,11,0.2)', fontSize: 12, color: 'var(--amber)', lineHeight: 1.6 }}>
